...
import {fetchTradingControl, fetchTradingEngineSnapshot, fetchTradingSetups, postStartTradingEngine, postTradingCommand, postTradingPreview, type SetupChoice, type TradePreview, type TradingControl} from '../api/client'
import type {TradingEngineSnapshot} from '../api/types'
import {TradeAuditPanel} from './TradeAuditPanel'
import {StopDraftControl} from './StopDraftControl'
...
export function TradingDesk({sessionDate}: {sessionDate: string}) {
  ...
  async function perform(action: () => Promise<void>) {
    ...
    try {await action()}
      ...
      setError(e instanceof Error ? e.message : 'Request failed. Check command history before retrying.')
