...
**Code baseline:** commit `CLEANSTART_COMMIT_SHA` on `codex/v1-acceptance-review`  
...
1. Approved release of commit `CLEANSTART_COMMIT_SHA` is what will run (or is already
...
```bash
set -euo pipefail
# Prefer the approved release that contains CLEANSTART_COMMIT_SHA.
cd /opt/nifty-radar/current

# 0) Confirm tree
git rev-parse HEAD   # must equal CLEANSTART_COMMIT_SHA (or release built from it)
