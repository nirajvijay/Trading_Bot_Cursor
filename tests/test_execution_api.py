...
class OldRouterCoexistenceTests(ExecutionApiTestCase):
    def test_the_old_trading_engine_routes_are_still_registered(self) -> None:
        paths = {r.path for r in self.app.routes}
        for path in (
            "/api/v1/trading-engine/status",
            "/api/v1/trading-engine/snapshot",
            "/api/v1/trading-engine/control",
            "/api/v1/trading-engine/setups",
        ):
