...
from api.schemas.checklist import ChecklistActivityResponse, GenerateResponse, PreMarketChecklistResponse
...
from api.services.checklist_activity import (
    ChecklistBusy, manual_operation, overlay_activity, read_activity, workflow_lock,
)
...
def premarket_checklist(
    ...
    before = read_activity(date)
    if activity_only:
        return {"session_date": date, "activity": before}
    ...
    activity = read_activity(date)
    overlay_activity(data, activity)
    # Serialize publication against writers; reject results spanning a mutation.
    try:
        with workflow_lock(shared=True):
            current = read_activity(date)
            if before == activity == current:
                write_checklist_cache(data)
    except ChecklistBusy:
        pass
    if before != activity and activity:
        # Force another lightweight-poll refresh if this scan crossed a transition.
        data["activity"] = {**activity, "revision": activity["revision"] + "-refresh"}
    return PreMarketChecklistResponse(**data).model_dump()
...
def generate_local_data(
    ...
    date = session_date or _today_ist()
    with manual_operation(task) as fd:
        success, message = run_local_generation(task, session_date=date, workflow_fd=fd)
        if not success:
