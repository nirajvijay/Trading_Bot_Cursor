    ...
    <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
        ...
        >
          ...
          <StatusField label="Prior session complete">
            {data.areas.historical_candles.prior_session_complete == null
              ? '—'
              : data.areas.historical_candles.prior_session_complete
                ? `yes (${data.areas.historical_candles.prior_session_completed_symbols ?? 0}/${data.areas.historical_candles.expected_count})`
                : `no (${data.areas.historical_candles.prior_session_completed_symbols ?? 0}/${data.areas.historical_candles.expected_count})`}
          </StatusField>
          <StatusField label="Bars on prior (min/avg/max)">
            {data.areas.historical_candles.bars_on_prior_min == null
              ? '—'
              : `${data.areas.historical_candles.bars_on_prior_min}/${data.areas.historical_candles.bars_on_prior_avg ?? '—'}/${data.areas.historical_candles.bars_on_prior_max ?? '—'}`}
          </StatusField>
          <StatusField label="Last candle on prior">
            {data.areas.historical_candles.last_candle_time_on_prior ?? '—'}
          </StatusField>
